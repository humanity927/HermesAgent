...
> 本仓库 `HermesAgent` 基于 [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent) 整理，记录 Windows 原生环境、Electron 桌面端和 Hermes Python 后端的本地运行结果。项目继续遵循上游 MIT 许可证。
...
下图来自本仓库在 Windows 环境中的实际运行结果。React 渲染进程通过 Electron 连接本地 `hermes serve` Python 后端，并已完成最小模型对话。

![Hermes Agent Windows 桌面端运行成功](docs/images/desktop-running.png)
